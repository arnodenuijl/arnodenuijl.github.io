...
/***/ function(module, exports, __webpack_require__) {
	...
	function createHttpInput(HTTP) {
	    var response$ = HTTP.mergeAll().tap(function (x) {
	...
	function createUserInput(DOM) {
	    // user input
	    return {
	        firstNameField$: DOM.select('#firstName').events('input').map(function (ev) { return ev.target.value; }),
	        lastNameField$: DOM.select('#lastName').events('input').map(function (ev) { return ev.target.value; }),
	...
	function model(userInput, httpInput$) {
	    ...
	    var firstName$ = userInput.firstNameField$.startWith("");
	    var lastName$ = userInput.lastNameField$.startWith("");
	    var model$ = rx_1.Observable
	        .combineLatest(firstName$, lastName$, function (firstName, lastName) { return ({ firstName: firstName, lastName: lastName, response: '' }); });
	    // if we get a response from the HTTP server we clear the model and show the return code
	    // let emptyModel$ = Observable.never();
	    var emptyModel$ = httpInput$.map(function (resp) { return ({ firstName: "", lastName: "", response: resp.statusCode }); });
	    var state$ = model$.merge(emptyModel$);
	    return state$;
	}
	function view(state$) {
	    var vtree$ = state$
	        .map(function (state) {
	        return dom_1.div([
	            dom_1.label('', 'First name:'),
	            dom_1.input('#firstName', { type: 'text', value: state.firstName }),
	            dom_1.label('', 'Last name:'),
	            dom_1.input('#lastName', { type: 'text', value: state.lastName }),
	            dom_1.p('', state.response),
	            dom_1.button('.saveButton', 'Opslaan'),
	        ]);
	    });
	    return vtree$;
	}
	function main(drivers) {
	    var DOM = drivers.DOM;
	    var HTTP = drivers.HTTP;
	    // User input 
	    var userInput = createUserInput(DOM);
	    // HTTP input
	    var httpInput$ = createHttpInput(HTTP);
	    // create state from user and http input
	    var state$ = model(userInput, httpInput$);
	    var vtree$ = view(state$);
	    // create the HTTP request from the last state sampled by the button click
	    var createUserRequest$ = state$
